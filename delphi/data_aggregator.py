...
from .time_discretization_utils import calculate_sazonality
...
class DataAggregator():
    ...
    def add_time_discretization(self, sazonality_type: str, window: int, frequency: int):
        '''
        Applies sazonality index as time discretization new column.
        ...
        window: (List[int], int)
            Sazonality window. Can be unique, passed as an int, or variable,
        ...
        if self.events_data is None:
            raise ValueError('Please inform events data using `add_events_data` method.')
        ...
        self.events_data[t_col] = calculate_sazonality(
            ts=self.events_data['ts'],
            sazonality_type=sazonality_type,
            window=window,
            frequency=frequency
        )
